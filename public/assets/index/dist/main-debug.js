define("xhub/index/1.0.0/main-debug", [ "angular/angularjs/1.1.5/angular-debug", "angular/angularjs/1.1.5/angular-resource-debug", "angularjs-debug" ], function(require, exports, module) {
    var angular = require("angular/angularjs/1.1.5/angular-debug");
    require("angular/angularjs/1.1.5/angular-resource-debug");
    "use strict";
    var index = angular.module("xhub.Index", [ "ngResource" ]);
    index.fixHttp = function($httpProvider) {
    ...
    index.config([ "$httpProvider", index.fixHttp ]);
            ...
            link: function($scope, $element, $attrs, ctrl) {
                ctrl.$parsers.unshift(function(viewValue) {
                    ...
                    if (viewValue && viewValue == pw) {
                        $scope.regForm.confirmPassword.$setValidity("confirmed", true);
                        return viewValue;
                    } else {
                        $scope.regForm.confirmPassword.$setValidity("confirmed", false);
                        return viewValue;
                    }
                });
            }
    ...
    index.c_CheckingLogin = function($scope, $resource, $routeParams) {
        var actions = {
            check: {
                method: "GET",
                responseType: "json"
        ...
        $scope.checkingLogin = function() {
            Checker.check({}, function(resp) {
                if (resp.email) {
                    window.location = "#/login/success?email=" + resp.email + "&nickName=" + resp.nickName;
                } else {
    ...
    index.controller("xhub.Index.CheckingLogin", index.c_CheckingLogin);
    index.c_CheckingLoginSuccess = function($scope, $resource, $routeParams) {
        $scope.email = $routeParams.email;
        $scope.nickName = $routeParams.nickName;
    };
    index.controller("xhub.Index.CheckingLoginSuccess", [ "$scope", "$resource", "$routeParams", index.c_CheckingLoginSuccess ]);
    index.c_Invite = function($scope, $resource, $routeParams, $http) {
        $scope.email = $routeParams.email || "";
        ...
        $scope.invite = function() {
            ...
            Inviter.invite({
                ...
                if (!resp.result) {
                    ...
                    window.location = "#/invite/success?email=" + $scope.email;
                }
            });
        };
    ...
    index.controller("xhub.Index.Invite", [ "$scope", "$resource", "$routeParams", "$http", index.c_Invite ]);
    index.c_InviteSuccess = function($scope, $resource, $routeParams, $http) {
        $scope.email = $routeParams.email;
    ...
    index.controller("xhub.Index.InviteSuccess", [ "$scope", "$resource", "$routeParams", "$http", index.c_InviteSuccess ]);
    index.c_Login = function($scope, $resource, $routeParams) {
        $scope.email = $routeParams.email || "";
        $scope.password = "";
        var actions = {
            login: {
        ...
        $scope.isvalid = function() {
            return $scope.email && $scope.email.length >= 8 && $scope.password && $scope.password.length >= 6;
        };
    };
    index.controller("xhub.Index.Login", [ "$scope", "$resource", "$routeParams", index.c_Login ]);
    index.c_Forgot = function($scope, $resource, $routeParams) {
        $scope.email = $routeParams.email || "";
    ...
    index.controller("xhub.Index.Forgot", [ "$scope", "$resource", "$routeParams", index.c_Forgot ]);
    index.c_Register = function($scope, $resource, $routeParams) {
    ...
    index.controller("xhub.Index.Register", [ "$scope", "$resource", "$routeParams", index.c_Register ]);
    index.c_Index = function($routeParams, $location) {
        var code = $location.$$search.code;
        var email = $location.$$search.email;
        if (code) {
            window.location = "index.html#/register?code=" + code + "&email=" + email;
        } else {
            window.location = "index.html#/invite";
        }
    };
    index.controller("xhub.Index.Index", [ "$routeParams", "$location", index.c_Index ]);
 ...
 */
    ...
    slice = [].slice, push = [].push, toString = Object.prototype.toString, _angular = window.angular, /** @name angular */
                    ...
                    return function(element, parent, after) {
                        ...
                        function beginAnimation() {
                            ...
                            if (polyfillStart) {
                                ...
                                $window.setTimeout(done, duration * 1e3);
                            } else {
                                done();
                            }
                        }
                ...
                function nodeLinkFn(childLinkFn, scope, linkNode, $rootElement, boundTranscludeFn) {
                    ...
                    if (newIsolateScopeDirective) {
                        ...
                        forEach(newIsolateScopeDirective.scope, function(definiton, scopeName) {
                            var match = definiton.match(LOCAL_REGEXP) || [], attrName = match[3] || scopeName, optional = match[2] == "?", mode = match[1], // @, =, or &
        ...
        return {
            ...
            compile: function(element, attr, linker) {
                return function($scope, $element, $attr) {
                    ...
                    var match = expression.match(/^\s*(.+)\s+in\s+(.*?)\s*(\s+track\s+by\s+(.+)\s*)?$/), trackByExp, trackByExpGetter, trackByIdFn, lhs, rhs, valueIdentifier, keyIdentifier, hashFnLocals = {
                        $id: hashKey
                    };
                    if (!match) {
                    ...
                    $scope.$watchCollection(rhs, function ngRepeatAction(collection) {
                        ...
                        nextCursor, // Same as lastBlockMap but it has the current state. It will become the
 ...
 */
    ...
    define("angular/angularjs/1.1.5/angular-debug", [], function() {
        return angular;
    });
})(window, document);
 ...
 * License: MIT
 */
define("angular/angularjs/1.1.5/angular-resource-debug", [ "angularjs" ], function(require) {
